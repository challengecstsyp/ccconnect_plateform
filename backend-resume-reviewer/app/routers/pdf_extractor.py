...
async def extract_cv_upload(file: UploadFile = File(...)):
    ...
    try:
        ...
        raise HTTPException(
            status_code=503, 
            detail=f"Ollama service is not available. Please ensure Ollama is running on http://localhost:11434. Install from https://ollama.com and run 'ollama pull gemma3:4b'"
        )
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
